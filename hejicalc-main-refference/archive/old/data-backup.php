...
//connection
$host_name = 'db761779987.hosting-data.io';
$database = 'db761779987';
$user_name = 'dbo761779987';
$password = '3HEJI@ll19';
...
$centSearch = "SELECT * FROM enharmonictable WHERE cents>=($centValue-$centRange) AND cents<=($centValue+$centRange) AND (hd>=$hdMin) AND (hd<=$hdMax) AND $filter3 AND $filter5 AND $filter7 AND $filter11 AND $filter13 AND $filter17 AND $filter19 AND $filter23 AND (acc<=$maxAcc) $sort;";
...
for ($i = 0; $i < $j; $i++) {
	...
	$five = $data[$i]["monzo5"] + $data[$i]["monzo17"];
